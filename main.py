...
if page == "Dashboard":
    ...
    if search:
        df = conn.query(f"SELECT * FROM inventory WHERE eq_id ILIKE '%%{search}%%'", ttl=0)
        if not df.empty:
...
# --- MANAGE INVENTORY ---
    ...
    with tab1:
        ...
        if eq_type == "Pump" and spare_type == "Seal":
            ...
            seal_oem = st.text_input("Seal OEM:", key="add_oem"); origin = st.selectbox("Origin:", ["OEM", "Locally made", "Locally refurbished"], key="add_orig")
        elif eq_type == "Compressor" and spare_type == "Valve":
            subtype = st.selectbox("Valve Type:", ["Suction valve", "Discharge valve"], key="add_sub"); origin = st.selectbox("Condition:", ["New", "Refurbished"], key="add_orig")
            if origin == "New": valve_oem = st.text_input("Valve OEM:", key="add_v_oem")
            else: vendor = st.text_input("Vendor Name:", key="add_ven"); ref_date = str(st.date_input("Refurbishment Date:", key="add_date"))
        elif spare_type == "Bearing": bearing_no = st.text_input("Enter Bearing Number:", key="add_bear"); origin = st.selectbox("Origin:", ["OEM", "Locally made"], key="add_orig")
        elif spare_type == "Mechanical spares": description = st.text_input("Enter Description:", key="add_desc"); origin = st.selectbox("Origin:", ["OEM", "Locally made"], key="add_orig")
    ...
    with tab2:
        ...
        if not df.empty:
            ...
            if selected_eq:
                eq_df = conn.query(f"SELECT * FROM inventory WHERE eq_id = '{selected_eq}'", ttl=0)
